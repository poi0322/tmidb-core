...
// 데이터베이스 함수들 (조직 DB용)
// 이 함수들은 각 조직의 데이터베이스에 적용되어야 합니다.
const orgFunctionsSQL = `
-- tmiDB 유틸리티 함수들
...
BEGIN
    -- 새 대상 생성
    INSERT INTO targets (name) VALUES (p_name) RETURNING target_id INTO new_target_id;
...
BEGIN
    ...
    -- 대상 존재 확인
    IF NOT EXISTS (SELECT 1 FROM targets WHERE target_id = p_target_id) THEN
