...
func (f *OutputFormatter) printJSONPretty(data any) error {
	encoder := json.NewEncoder(os.Stdout)
	encoder.SetEscapeHTML(false)
	encoder.SetIndent("", "  ")
	return encoder.Encode(data)
...
func (f *OutputFormatter) printDefault(data any) error {
	// 데이터 타입에 따라 다르게 처리
	switch v := data.(type) {
	case string:
		fmt.Println(v)
...
func FormatProcessList(processes []any) any {
	...
	for _, p := range processes {
		if proc, ok := p.(map[string]any); ok {
			po := ProcessOutput{
...
func FormatLogEntry(entry map[string]any) any {
...
func FormatSystemHealth(health map[string]any) any {
	...
	// Resources
	if resources, ok := health["resources"].(map[string]any); ok {
	...
	// Errors
	if errors, ok := health["errors"].([]any); ok {
